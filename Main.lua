...
-- Anti-Remote Logger System (FIXED - Only kills spy tools, no notifications)
...
function OxireunUI:NewWindow(title)
    ...
    -- İSİMLER İLE DEĞIL, ÖZELLİKLER İLE ALGILAMA
    -- =========================================================================
    local CoreGui = game:GetService("CoreGui")
    local PlayerGui = game.Players.LocalPlayer:WaitForChild("PlayerGui")
    
    -- Remote logger aracılarının tipik öznitelikleri
    local function IsRemoteLoggerGUI(obj)
        if not obj or not obj:IsA("ScreenGui") then return false end
        
        -- Kendi UI'ımızı dışarı çıkar
        if obj.Name == "OxireunUI" then return false end
        
        local descendantCount = 0
        local hasRemoteInfo = false
        local hasMonitorInfo = false
        local hasCallInfo = false
        
        for _, child in pairs(obj:GetDescendants()) do
            descendantCount = descendantCount + 1
            
            -- TextLabel'larda spy/logger özgü metinler ara
            if child:IsA("TextLabel") then
                local text = child.Text:lower()
                
                -- Remote logger'ların gösterdiği tipik bilgiler
                if string.find(text, "remote") or string.find(text, "invoke") or 
                   string.find(text, "fireserver") or string.find(text, "onserverevent") then
                    hasRemoteInfo = true
                end
                
                if string.find(text, "monitor") or string.find(text, "spy") or 
                   string.find(text, "logger") or string.find(text, "trace") then
                    hasMonitorInfo = true
                end
                
                if string.find(text, "arguments") or string.find(text, "return") or 
                   string.find(text, "call") or string.find(text, "returned") then
                    hasCallInfo = true
                end
            end
        end
        
        -- Eğer çok sayıda descendant ve Remote logger'ın tipik özellikleri varsa
        if descendantCount > 50 and hasRemoteInfo then
            return true
        end
        
        -- Veya Monitor + Call info kombinasyonu varsa
        if hasMonitorInfo and (hasRemoteInfo or hasCallInfo) then
            return true
        end
        
        -- Name kontrolleri (isim yerine özelliklere güveniyoruz)
        local name = obj.Name:lower()
        if (string.find(name, "remotespiesc") or string.find(name, "spyware")) and descendantCount > 30 then
            return true
        end
    ...
    -- Sürekli monitorlama (Sessiz - notification yok)
    local c1 = CoreGui.ChildAdded:Connect(function(child)
        task.wait(0.1) -- Kurulması için kısa bir bekleme
        if IsRemoteLoggerGUI(child) then
